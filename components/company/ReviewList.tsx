  ...
  return (
    <ul className="flex flex-col gap-4">
      {data.items.map((review) => {
        // BE-67 — server-computed, correct for anonymous own-reviews too
        // (comparing `review.userId` to the caller's own id directly can
        // never match once `userId` is nulled for anonymity; found live
        // via TC-FU30-03).
        const isMine = review.isMine;
          ...
          <li key={review.id} className="rounded-lg border border-border bg-surface p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Avatar name={review.authorDisplayName ?? undefined} anonymous={!review.authorDisplayName} size="sm" />
                <span className="text-sm font-medium text-text">
                  {review.authorDisplayName ?? text.common.anonymousAuthorLabel}
                </span>
              </div>
              <span className="text-xs text-text-muted">{formatDate(review.createdAt, lang)}</span>
            </div>

            <div className="mt-2 flex items-center gap-1" aria-label={text.companies.reviews.ratingAriaLabel(review.rating)}>
              {[1, 2, 3, 4, 5].map((value) => (
                <StarIcon
                  key={value}
